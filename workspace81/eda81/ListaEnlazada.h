...
public :
	...
	bool inserta (E& elem, int pos=_lon)
	{

		Nodo* unNodo(elem);
		if (pos==_lon) insertaUltimo(unNodo);
		else
			if (pos==0) insertaPrimero(unNodo);
			else{
				unNodo=dameEnesimo(pos);
				this->insertaElem(pos,)
			}
...
protected:
	Nodo* dameEnesimo(int n)
	{
		Nodo* puntero=_primero;
		if (n>=_lon || n<0) return NULL;
		for (int i=1;i<_lon;i++)
		{
