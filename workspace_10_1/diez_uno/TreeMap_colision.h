...
#ifndef __TREEMAP_colision_H
#define __TREEMAP_colision_H
...
public:
	...
	/** Constructor copia */
	TreeMap_colision(const TreeMap_colision<Clave, Valor> &other) : _ra(NULL) {
		copia(other);
	}

	/** Operador de asignaci�n */
	TreeMap_colision<Clave, Valor> &operator=(const TreeMap<Clave, Valor> &other) {
		if (this != &other) {
			libera();
			copia(other);
		}
...
protected:
	 ...
	 */
	TreeMap_colision(Nodo *raiz) : _ra(raiz) {
	}
	...
	void copia(const TreeMap_colision &other) {
