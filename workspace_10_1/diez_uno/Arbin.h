...
inline int dameMax ( int a, int b)
{
	if (a>b) return a;
	else
		return b;
}
inline int dameMin (int a, int b)
{
	if (a<b) return a;
	else
		return b;
...
// siempre  que puedo actualizo max y min
//min,max estritamente de salida
bool esOrdenadoAux(const Arbin<int> & unArbol,int& unMin, int& unMax)
	{

		// si es vacío
		if (unArbol.esVacio()) return true;

		//si es hoja (hijodr e hijoiz vacios)
		...
		//si hijoDr es vacio
		//compruebo si hijo izquierdo está ordenado
		//y compruebo que el máximo es la raiz y
		//el minimo es el minimo del hijo izquierdo

		if (unArbol.hijoDr().esVacio())
		{
			if (esOrdenadoAux(unArbol.hijoIz(),unMin,unMax))
			{
				return (unMin==dameMin(unArbol.raiz(),unMin) &&
						unArbol.raiz()==dameMax(unArbol.raiz(),unMax) );
			}
		}

		//si hijoIz es vacío
		//compruebo si hijo derecho está ordenado
		//y compruebo que el minimo es la raíz
		//y el máximo es el máximo del hijo derecho
		else if (unArbol.hijoIz().esVacio)
			if (esOrdenadoAux(unArbol.hijoDr(),unMin,unMax))
			{
				return (unArbol.raiz()==dameMin(unArbol.raiz(),unMin) &&
						unMax==dameMax(unArbol.raiz(),unMax));
			}
		...
		//si es raiz
			else
			{
				...
				iz= esOrdenadoAux(unArbol.hijoIz(),unMin,unMax);
				iz= iz && (unMin==dameMin(unArbol.raiz(),unMin) &&
						unArbol.raiz()==dameMax(unArbol.raiz(),unMax) );
				...
				der= esOrdenadoAux(unArbol.hijoDr(),unMin,unMax);
				der=der && (unArbol.raiz()==dameMin(unArbol.raiz(),unMin) &&
						unMax==dameMax(unArbol.raiz(),unMax));
