...
for (int i=0;i<unString.length();i++)
	if ( unString [i]=='(' || unString [i]=='{' || unString [i]=='[')
		unStack.push(unString[i]);
	else if (unString [i]==')' || unString [i]=='}' || unString [i]==']')
	{
		if (unStack.empty()) return false;
		else if (!pareja(unStack.top(),unString[i])) return false;
		else
			unStack.pop();
	}

return (unStack.empty());
}
int main()
{
	string unString="";
